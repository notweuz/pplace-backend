...
func (h *UserHandler) GetSelfInfo(c *fiber.Ctx) error {
	...
	if err != nil {
		return model.NewHttpError(fiber.StatusInternalServerError, "Failed to fetch user info", []string{err.Error()})
	}
...
func (h *UserHandler) CreateUser(c *fiber.Ctx) error {
	...
	if err := c.BodyParser(&user); err != nil {
		return model.NewHttpError(fiber.StatusBadRequest, "Invalid request body", []string{err.Error()})
	}
...
func (h *UserHandler) UpdateUser(c *fiber.Ctx) error {
	...
	if errors := validation.ValidateDTO(&updateData); errors != nil {
		...
		return model.NewHttpError(fiber.StatusBadRequest, "Validation failed", stringErrors)
	}

	if updateData.Username == "" && updateData.Password == "" {
		return model.NewHttpError(fiber.StatusBadRequest, "At least one field (username or password) must be provided", nil)
	}

	currentUser, err := h.service.GetSelfInfo(c)
	if err != nil {
		return model.NewHttpError(fiber.StatusUnauthorized, "Unauthorized", []string{err.Error()})
	}
...
func (h *UserHandler) GetUserByID(c *fiber.Ctx) error {
	...
	if err != nil {
		return model.NewHttpError(fiber.StatusBadRequest, "Invalid user ID", []string{err.Error()})
	}

	user, err := h.service.GetByID(c.Context(), uint(id))
	if err != nil {
		return model.NewHttpError(fiber.StatusNotFound, "User not found", []string{err.Error()})
	}
...
func (h *UserHandler) GetUserByUsername(c *fiber.Ctx) error {
	...
	if username == "" {
		return model.NewHttpError(fiber.StatusBadRequest, "Username is required", nil)
	}
